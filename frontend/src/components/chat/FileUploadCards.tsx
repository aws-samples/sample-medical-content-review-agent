...
export function FileUploadCards({
  ...
  const isAccepted = (file: File) =>
    ACCEPTED_TYPES.includes(file.type) || ACCEPTED_EXTENSIONS.split(",").some((ext) => file.name.toLowerCase().endsWith(ext));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-950/40 dark:to-blue-950/40 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center">
            ...
            {documentFile && (
              <button onClick={(e) => { e.stopPropagation(); onDocumentChange(null); }} className="text-gray-500 hover:text-red-600 transition-colors">
        ...
        >
          <input ref={documentInputRef} type="file" accept={ACCEPTED_EXTENSIONS} onChange={(e) => { const f = e.target.files?.[0]; if (f && isAccepted(f)) onDocumentChange(f); }} className="hidden" />
          <div className="text-center">
            ...
            {documentFile ? (
              <div>
                <p className="text-lg font-semibold text-indigo-600 mb-2">✓ {documentFile.name}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Click to change or drop new file</p>
              </div>
            ) : (
              <div>
                <p className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">Drop file or click to select</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">The medical content document to review</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Supported formats: PDF, TXT, CSV, MD, DOC, DOCX</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/40 dark:to-emerald-950/40 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center">
            ...
            {referenceFiles.length > 0 && (
              <button onClick={(e) => { e.stopPropagation(); onReferenceFilesChange([]); }} className="text-gray-500 hover:text-red-600 transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            )}
          </div>
        ...
        >
          <input ref={referenceInputRef} type="file" accept={ACCEPTED_EXTENSIONS} multiple onChange={(e) => { const files = Array.from(e.target.files || []).filter(isAccepted); if (files.length) onReferenceFilesChange([...referenceFiles, ...files]); }} className="hidden" />
          <div className="text-center w-full">
            ...
            <p className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">
              {referenceFiles.length > 0 ? `✓ ${referenceFiles.length} reference(s) added` : "Drop references or click to select"}
            </p>
            {referenceFiles.length === 0 && (
              <p className="text-xs text-gray-500 dark:text-gray-400">Source documents to verify claims against (PDF, TXT, CSV, MD, DOC, DOCX)</p>
            )}
            {referenceFiles.length > 0 && (
              <div className="mt-3 max-h-32 overflow-y-auto px-2">
                {referenceFiles.map((file, idx) => (
                  <div key={idx} className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 rounded px-2 py-1 mb-1 flex items-center gap-1">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Multiple files supported</p>
          </div>
        </div>
      </div>
    </div>
